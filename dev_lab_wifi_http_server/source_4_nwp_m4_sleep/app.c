...
static void application_start(void *argument)
{
  ...
  sl_si91x_gpio_pin_config_t sl_button1_pin_config = { { button_btn1.port, button_btn1.pin }, GPIO_INPUT };
  ...
  // reduces packets nwp needs to wake for
  ...
  while (1) {
    ...
    osDelay(1000);
    sl_si91x_power_manager_add_ps_requirement(SL_SI91X_POWER_MANAGER_PS3);
    // reinitialise button 1 after sleep
    sl_gpio_driver_init();
    sl_gpio_set_configuration(sl_button1_pin_config);
    // update data    
    seconds++;
